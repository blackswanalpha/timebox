    ...
    return (
        <div className="relative w-full" ref={dropdownRef}>
            ...
            <button
                ...
                className={`w-full h-14 px-5 bg-white dark:bg-slate-800 border rounded-2xl flex items-center justify-between transition-all duration-200 shadow-sm ${isOpen ? 'ring-4 ring-amber-500/10 border-amber-500' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'
                    }`}
            >
                <div className="flex items-center gap-3 overflow-hidden">
                    <div className={`p-2 rounded-lg ${selectedTask ? 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'bg-slate-50 dark:bg-slate-900/40 text-slate-400'}`}>
                    ...
                    >
                        <div className="max-h-64 overflow-y-auto custom-scrollbar py-2">
                            <button
                                ...
                                className={`w-full px-5 py-3 flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors ${!selectedTaskId ? 'bg-amber-50/50 dark:bg-amber-900/20' : ''}`}
                            >
                                <span className="text-sm font-bold text-slate-600 dark:text-slate-300">Default Focus Session</span>
                                {!selectedTaskId && <CheckCircleIcon className="h-5 w-5 text-amber-500" />}
                            </button>
                                ...
                                activeTasks.map((task) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full px-5 py-3 flex flex-col gap-1 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors ${selectedTaskId === task.id ? 'bg-amber-50/50 dark:bg-amber-900/20' : ''}`}
                                        >
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate pr-2">{task.title}</span>
                                                {selectedTaskId === task.id && <CheckCircleIcon className="h-5 w-5 text-amber-500 flex-shrink-0" />}
                                            </div>
